...
Docker Image Puller 是一个方便的工具，用于从 Docker 仓库拉取镜像，支持断点续传、多架构和缓存管理。该工具采用 MIT 许可证，开放源代码，方便用户根据需要进行定制和扩展。
...
- **智能缓存**：不同镜像使用独立缓存目录，支持查看和清理缓存。
- **多架构支持**：支持多种架构（如 `amd64`、`arm64v8`），满足不同环境需求，尤其是 arm64 内网服务器。
...
| `-a, --arch` | 架构，默认：amd64，常见：amd64, arm64v8 等 |
...
📦 标签：latest
📦 架构：amd64
📁 缓存目录：C:\Users\xxx\.docker-pull-cache\library_alpine_latest_amd64
📥 开始下载...
✅ 镜像已保存为: library_alpine_latest_amd64.tar
💡 导入命令: docker load -i library_alpine_latest_amd64.tar
```
...
# 下载指定架构镜像
DockerPull.exe -i alpine:latest -a arm64v8

# 下载私有仓库镜像
DockerPull.exe -i harbor.example.com/library/nginx:1.26.0 -u admin -p password

# 查看缓存
DockerPull.exe --list-cache

# 清除缓存
DockerPull.exe --clear-cache
...
工具会在用户目录下创建缓存目录 `~/.docker-pull-cache/`，每个镜像使用独立的子目录。
...
📁 缓存目录: C:\Users\xxx\.docker-pull-cache
============================================================
library_alpine_latest_amd64: ✅ 完成 (5.2 MB)
   镜像: library/alpine:latest
   架构: amd64

library_nginx_latest_amd64: ⏳ 3/5 (120.5 MB)
   镜像: library/nginx:latest
   架构: amd64
============================================================
共 2 个缓存，总大小: 125.7 MB
```
...
- **架构灵活**：支持 `amd64` 和 `arm64v8` 架构，适应多种环境。
- **易于使用**：单文件脚本，无需复杂配置。
- **开放源代码**：自由定制和扩展。
...
**Q**: 支持哪些架构？  
**A**: 支持 Docker Hub 上所有 Linux 架构，常见：`amd64`、`arm64v8`、`armv7` 等。
...
**Q**: 缓存文件在哪里？  
**A**: 缓存目录位于 `~/.docker-pull-cache/`，可使用 `--list-cache` 查看。
